...
*/
void part1b() {
  // Bug 1: Comparison of pointer with integer zero. Use i instead of *i. 
	uint8_t i;

  // Bug2: initial number should be assigned
  uint8_t ledVar = 0;

  // Bug 3: Initialize i should use '=' instead of '=='
  // Bug 4: should use 'i++' instead of '++i' 
  // Bug 5: unit8_t range from 0 to 255
	for (i = 30; i <= 50; i++) {

    //Bug 5: use write function to blink the LED light
    led2.write(ledVar);

		// toggle LED
		ledVar = !ledVar;

    //Bug 5: wait_us only accept int datatype. Use wait_ms for a longer time pause instead.
		wait_ms(500);
	}
}
...
void part1c() {
  ...
  //uint16_t hexLiteral1 = 0x00001234;
  printf("Original hexLiteral:%x\n", hexLiteral);
  printf("Eight digit hexLiteral:%08x\n", hexLiteral);
  printf("Eight digit with 0x hexLiteral:0x%#08x\n", hexLiteral);
...
*/
void part2() {
  // Answere: 'Welcome to RTES !'
  // uint8_t decodedString[16] = {'W', 'e', 'l', 'c', 'o','m', 'e', ' ', 't', ''o, ' ', 'R', 'T', 'E', 'S', '', '!'};
	uint8_t i;
...
void extension () {
  ...
  // uint8_t decoded_message[13] = {'I', ' ', 'l', 'o', 'v', '\377', ' ', 'p', '\356', 'z', 'z', '\376', '.'};
  // Personal Guess: I love pizza.
